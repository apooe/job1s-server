...
const update = async (recruiter) => {
  try {
    const newRecruiter = await RecruiterModel.findOneAndUpdate(
      { _id: recruiter._id },
      recruiter,
      { new: true }
    );
...
const searchJobPosts = async (jobName) => {
  ...
  if (jobName) {
    ...
    const locationRegex = {
      "jobPosts.location": { $regex: `.*${formattedJobName}.*`, $options: "i" },
    };
    const companyRegex = {
  ...
  try {
    const recruiters = await RecruiterModel.find({
      jobPosts: { $exists: true, $not: { $size: 0 } },
    }).or(jobQuery);
    return recruiters.map((recruiter) => {
      recruiter.password = undefined;
      recruiter.jobPosts = recruiter.jobPosts.filter(
        (j) =>
          ...
          j.relatedJobs.find((r) => r.toLowerCase().includes(formattedJobName))
      ); //j.title.toLowerCase()
      return recruiter;
    });
  } catch (e) {
...
const findRelatedJobSeeker = async (recruitedId) => {
  try {
    ...
    const userRelated = await UserModel.find({
      job: { $in: allRelatedJobs },
    }).lean();
