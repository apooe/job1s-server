...
const changePassword = async (id, body) => {
  try {
    ...
    const user =
      (await getById(id)) || (await RecruiterModel.findOne({ _id: id }));
...
const checkCodeReset = async (code) => {
  const user =
    (await UserModel.findOne({ resetCode: code })) ||
    (await RecruiterModel.findOne({ resetCode: code }));
  if (!user) {
...
const searchProfiles = async (jobName) => {
  ...
  if (jobName) {
    ...
    const locationRegex = {
      city: { $regex: `.*${formattedJobName}.*`, $options: "i" },
    };
    const firstnameRegex = {
      firstname: { $regex: `.*${formattedJobName}.*`, $options: "i" },
    };
    const lastnameRegex = {
      lastname: { $regex: `.*${formattedJobName}.*`, $options: "i" },
    };

    const collegeRegex = {
      "education.collegeName": {
        $regex: `.*${formattedJobName}.*`,
        $options: "i",
      },
    };
    const degreeRegex = {
      "education.degree": { $regex: `.*${formattedJobName}.*`, $options: "i" },
    };
  ...
  try {
    ...
    if (profiles.length) {
      users.push(
        ...profiles
          .filter((p) => !userIds.has(p.userId?._id))
          .map((p) => p.userId)
      );
    }
    return users.map((user) => {
